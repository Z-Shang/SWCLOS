...
;;;
;;; The standard slot definition in CLOS (standard-slot-definition) is also a CLOS object 
...
;;; which is a specialized class of <standard-direct-slot-definition>. In addition to CLOS 
...
;;;
#|
;; To get a newest option value of type in current situation, use <slot-definition-type>. 
...
(defmethod shared-initialize :after ((slotd Property-direct-slot-definition) slot-names
  ...
  (cond ((eq slot-names t) ; when first
         (cond ((eq name (slot-definition-name slotd))
