            ...
            collect (cons role 
                          (loop for filler in fillers
                              collect (cond ;((c2cl:typep filler '|rdf|:|inLang|)
                                            ...
                                            ((rsc-object-p filler) (node-name filler))
                                            (t filler)))))))
...
(defun tree->list (mop fn visited)
  ...
  (cond ((eq mop (find-class '|rdfs:Class|)) nil)
        ((member mop visited) (node-name mop))
        ((not (%instance-p mop))
         (push mop visited)
         `(,(node-name mop) ,@(funcall fn mop visited)
             ,@(mapcar #'node-name (class-direct-instances mop))))))
...
(defun (setf ->) (value mop &rest roles)
  ...
  (cond ((null (slot-exists-p mop (node-name (car roles)))) nil)
        ((null (cdr roles)) (setf (slot-value mop (node-name (car roles))) value))
        ((and (slot-boundp mop (node-name (car roles)))
              (slot-value mop (node-name (car roles))))
         (setf (apply #'-> (slot-value mop (node-name (car roles))) (cdr roles)) value))
        (t ;slot exists but unbound with multiple roles
         (let ((slotd (find (node-name (car roles)) (class-slots (class-of mop))
                            :key #'slot-definition-name)))
           (format t "~%SLOTD=~S" slotd)
           (when slotd
             (setf (slot-value mop (node-name (car roles)))
...
(defun %setfvalue (value type roles)
  (cond ((null roles) value)
        (t (unless (class-finalized-p (symbol-value type))
           ...
           (let ((slotd (find (node-name (car roles)) (class-slots (symbol-value type))
                              :key #'slot-definition-name)))
             (format t "~%slotd=~S" slotd)
             (when slotd
  ...
  (remove-duplicates 
   (append (collect-direct-subtypes class)
           (loop for sub in (class-direct-subclasses class)
               append (cond ((eq sub (find-class '|rdfs:Class|)) (list |rdfs|:|Class|))
...
(defun collect-domain-properties (subject &optional (prop |rdf|:|Property|))
  ...
  (append (loop for ins in (class-direct-instances prop)
              when (let ((domain (get-domain ins)))
                     (or (and (symbolp domain) (eq (node-name subject) domain))
...
(defun collect-range-properties (object &optional (prop |rdf|:|Property|))
  ...
  (append (loop for ins in (class-direct-instances prop)
              when (let ((range (get-range ins)))
                     (or (and (symbolp range) (eq (node-name object) range))
  ...
  (let ((pkg
    ...
    (labels ((%all-individuals (cls)
                               (when (and (not (eql cls (load-time-value (find-class '|rdfs|:|Class|))))
                                          (not (eql cls (load-time-value (find-class '|rdfs:Class|)))))
