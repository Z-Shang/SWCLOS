		...
		(cond ((symbol-equal name 'initform) ; old: (eq name 'excl::initform)
		      ...
		      ((symbol-equal name 'initfunction)
		       (and (slot-definition-initfunction slotd)
			    (not (eq (slot-definition-initfunction slotd) #'false))))
		      ...
		      ((or (eq name 'documentation)
		       ...
		       (slot-value slotd name))
		      ((symbol-equal name 'fixed-index) ; this should be ACL only
		       nil)
		      ((symbol-equal name 'allocation)
		       (not (eq :instance (slot-value slotd name))))
		      (t t)))
      append (cond ((symbol-equal name 'name)
		   ...
		   (t
		    (let ((initargs (car (slot-definition-initargs facetd))))
		      (when (and initargs
				 (symbolp initargs)
				 (eq (symbol-package initargs) (find-package :keyword))
...
(defmethod change-class ((instance rdfs:|Resource|) (new-class rdfs:|Class|) &rest initargs)
  (let ((old-class (class-of instance)))
    ...
    (cond ((and (shadowed-class-p old-class) (not (shadowed-class-p new-class)))
          ...
          ((c2cl:typep old-class 'shadowed-class)
           (let ((shadow (make-shadow old-class (cons new-class (mclasses instance)))))
             (apply #'call-next-method instance shadow initargs)))
          ((c2cl:typep new-class 'shadowed-class)
          ...
          ; )
          ((c2cl:typep instance 'shadowed-class)
           (warn "Attempting to refine ~S to ~S" instance new-class) ; by smh
           (call-next-method)  ; by smh
           )
      ...
      (loop for slotd in (class-direct-slots old-class)
          do (let ((name (slot-definition-name slotd)))
               (let ((eslotd (find name (class-slots old-class)
                                   :key #'slot-definition-name)))
                 (when (and (c2cl:typep eslotd 'Property-effective-slot-definition)
