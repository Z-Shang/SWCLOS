  ...
  (let ((lang-env lang-env))  ; this is effective in read-in-lang-env
    (etypecase form
      ...
      (rdfs:|Resource| (assert (and (not (eq role t)) (not (eq role nil))))
                     (error "Cant happen!")
                     (when (node-name form)
                       (push `(,(node-name form) line ,*line-number*) *referenced-resources*))
                     form)
                                    ...
                                    do (add-form
                                        (cond ((consp one)                                    ; maybe a form
                                               (cond ((eq (car one) 'owl:|Thing|)
                                                     ...
                                                     (t (append one `((rdf:|type| ,(node-name obj)))))))
        ...
        (cond ((null range) obj)
              ((atom range)
               (cond ((eq obj rdfs:|Class|) obj)
                     ...
                     (t (warn "Range entailX4 by ~S: ~S rdf:type ~S." role obj (node-name range))
                        (change-class obj range))))
              ((consp range)
               (cond ((every #'(lambda (cls) (c2cl:typep obj cls)) range) obj)
                     (t (warn "Range entailX5 by ~S: ~S rdf:type ~S." role obj (node-name range))
                        (change-class obj range))))
              ((error "Cant happen!"))))
    obj))
...
(defun make-coined-name (types)
  ...
  (when (length=1 types) (return-from make-coined-name (node-name (car types))))
  (setq types (remove-if #'(lambda (ty) (eql (symbol-package (node-name ty)) (find-package :rdf)))
                         types))
  (when (length=1 types) (return-from make-coined-name (node-name (car types))))
  (setq types (remove-if #'(lambda (ty) (eql (symbol-package (node-name ty)) (find-package :rdfs)))
                         types))
  (when (length=1 types) (return-from make-coined-name (node-name (car types))))
  (let ((types-1 
         (remove-if #'(lambda (ty) (eql (symbol-package (node-name ty)) (find-package :owl)))
                    types)))
    (when (length=1 types-1) (return-from make-coined-name (node-name (car types-1))))
    (let ((namelist (mapcar #'node-name (or types-1 types))))
...
(defun accumulate-defined-name (name)
  ...
  (cond ((consp name)
        ...
        ((rsc-object-p name) (push `(,(node-name name) line ,*line-number*) *defined-resources*))
        ((error "Cant happen!"))))
              ...
              (cond ((and (rdf-metaclass-p class) (rdf-metaclass-p typ)))
                    ((and (rdf-metaclass-p class) (strict-class-p typ))
                     ...
                     (reinitialize-instance
                      typ
                      :direct-superclasses (cons *meta* (class-direct-superclasses typ))))
                    ((and (rdf-metaclass-p class) (rdf-instance-p typ))
                     (cerror "Anyway change it to class ?" "~S is not metaclass!" (class-of typ))
                     (reinitialize-instance
                      (class-of typ)
                      :direct-superclasses (cons *meta* (class-direct-superclasses (class-of typ)))))
                    ((error "Cant happen!"))))
...
(defmethod ensure-meta-absts-using-class ((meta rdfs:|Class|) (class rdfs:|Class|) slot-forms domains)
  (flet ((ensure-meta (fil) (cond ((rdf-metaclass-p fil) fil)
                                  ((rdf-class-p fil)
                                   (reinitialize-instance
                                    fil
                                    :direct-superclasses (cons *meta* (class-direct-superclasses fil))))
                                  (t (cerror "Anyway metaclassify ~S?" "~S is not metaclass!" (class-of fil))
                                     (reinitialize-instance
                                      (class-of fil)
                                      :direct-superclasses (cons *meta* (class-direct-superclasses (class-of fil))))
                                     fil)))
             ...
             (cond ((every #'(lambda (cls) (c2cl:typep obj cls)) classes)
                   ...
                   ((%instance-p obj)                          ; instance to class
                    ...
                    (ensure-multiple-classes
                     classes
                     (apply #'ensure-class-using-class obj name
                   ...
                   ((rdf-metaclass-p (car classes))   ; class to class
                    ...
                    (cond ((and (null absts) (null initargs))
                          ...
                          ((and (null name) absts)
                           (ensure-multiple-classes
                            classes
                            (apply #'ensure-class-using-class obj nil
                                   :direct-superclasses absts
                                   :metaclass (car classes)
                                   initargs)))
            ...
            ((null name)                                        ; anonymous
             ...
             (set-iri-value
              (ensure-multiple-classes
               classes
               (if absts
                   (apply #'ensure-class-using-class nil nil
                          :direct-superclasses absts
                          :metaclass (car classes)
                          initargs)
                 (apply #'ensure-class-using-class nil nil
...
(defun add-instance (classes instance &optional slots)
  ...
  (let ((name (etypecase instance
                ...
                (rdfs:|Resource| (node-name instance))))
          ...
          (loop for prop in new-props with new-domain
              ...
              do (reinitialize-instance
                  ...
                  `((:name ,prop :initargs (,prop) :type ,(or (get-range (symbol-value prop)) t)
                           ...
                           :subject-type ,class)))
                (setq operated t))
          (when operated (finalize-inheritance class)))))))
...
(defun collect-owl-role-name-if (test obj)
  (loop for slotd in (class-slots (class-of obj))
      when (and (c2cl:typep slotd 'Property-effective-slot-definition)
                (funcall test (symbol-value (slot-definition-name slotd))))
