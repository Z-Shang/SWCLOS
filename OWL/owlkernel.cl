...
(defun owl-thing-p (obj)
  ...
  (and (standard-instance-p obj)
       ...
       (not (eq (node-name obj) 'owl:|Nothing|))))
    ...
    (owl:|ObjectProperty|
     (loop for domain in (mklist (and (slot-boundp instance 'rdfs:|domain|)
         ...
         do (loop for slotd in (class-direct-slots domain)
                when (eq (node-name instance) (slot-definition-name slotd))
  ...
  (cond ((and (slot-exists-p obj 'name)
        ...
        ((and (slot-exists-p obj 'owl:|onProperty|)
         ...
         (print-unreadable-object (obj stream :type nil)
           (format stream "  ~S.~S"
             (node-name (slot-value obj 'owl:|onProperty|))
             (or (node-name (slot-value obj 'owl:|allValuesFrom|))
                 (get-form (slot-value obj 'owl:|allValuesFrom|))))))
        (t (call-next-method))))

(defmethod node-name ((object owl:|allValuesFromRestriction|))
...
(defmethod print-object ((obj owl:|someValuesFromRestriction|) stream)
  (cond ((and (slot-exists-p obj 'owl:|onProperty|)
         ...
         (print-unreadable-object (obj stream :type nil)
           (format stream "  ~S.~S"
             (node-name (slot-value obj 'owl:|onProperty|))
             (or (node-name (slot-value obj 'owl:|someValuesFrom|))
                 :anonymous))))
        (t (call-next-method))))

(defmethod node-name ((object owl:|someValuesFromRestriction|))
  (cond ((and (slot-boundp object 'name) (slot-value object 'name)))
...
(defmethod node-name ((object owl:|hasValueRestriction|))
  (cond ((and (slot-boundp object 'name) (slot-value object 'name)))
...
(defmethod print-object ((obj owl:|cardinalityRestriction|) stream)
  (cond ((slot-value obj 'owl:|onProperty|)
         (print-unreadable-object (obj stream :type nil)
           (prin1 (node-name (slot-value obj 'owl:|onProperty|)) stream)
